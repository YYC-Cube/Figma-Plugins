...
// 应用自定义效果
function applyCustomEffect(customEffect) {
  const nodes = figma.currentPage.selection;
  if (nodes.length === 0) {
    figma.notify('请先选择元素');
    return;
...
// 应用效果到节点
function applyEffectToNode(node, effect) {
  switch (effect.type) {
    case 'blur':
...
// 应用模糊效果
function applyBlurEffect(node, params) {
  if ('effects' in node) {
    const blurEffect = {
      type: 'BLUR',
      visible: true,
...
// 应用阴影效果
function applyShadowEffect(node, params) {
  if ('effects' in node) {
    const shadowEffect = {
      type: 'DROP_SHADOW',
...
// 应用光晕效果
function applyGlowEffect(node, params) {
  if ('effects' in node) {
    const glowEffect = {
      type: 'INNER_SHADOW',
      visible: params.visible,
...
// 应用渐变效果
function applyGradientEffect(node, params) {
  if ('fills' in node) {
    let gradient;
    ...
    if (params.type === 'LINEAR') {
      gradient = {
        ...
        gradientStops: params.colors.map((color, index) => ({
          color: color,
...
// 应用噪点效果
function applyNoiseEffect(node, params) {
  // 噪点效果实现
...
// 应用扭曲效果
function applyDistortionEffect(node, params) {
  // 扭曲效果实现
...
// 应用复古效果
function applyVintageEffect(node, params) {
  // 复古效果实现
...
// 应用漫画效果
function applyComicEffect(node, params) {
  // 漫画效果实现
...
// 应用像素效果
function applyPixelEffect(node, params) {
  // 像素效果实现
...
// 移除效果
function removeEffect(nodeId, effectId) {
