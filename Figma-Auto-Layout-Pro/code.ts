...
// 应用智能布局
function applySmartLayout(ruleId: string) {
...
// 批量布局操作
function batchLayout(operation: string, params: any) {
  const selection = figma.currentPage.selection;
  if (selection.length === 0) {
...
// 调整间距
function adjustSpacing(amount: number, direction: 'increase' | 'decrease') {
  const selection = figma.currentPage.selection;
  if (selection.length === 0) {
...
// 对齐元素
function alignElements(alignType: string) {
  const selection = figma.currentPage.selection;
  if (selection.length < 2) {
...
// 获取对齐类型文本
function getAlignTypeText(alignType: string) {
...
// 创建网格
function createGrid(columns: number, rows: number, gap: number) {
  // 模拟创建网格
  setTimeout(() => {
    figma.ui.postMessage({
      type: 'grid-created',
