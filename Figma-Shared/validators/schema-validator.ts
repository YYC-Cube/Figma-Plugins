import { DatabaseSchema, Table } from '../types/database.types';
import { ValidationResult, ValidationIssue, ValidationConfig } from '../types/validation.types';
...
export class SchemaValidator {
  ...
  validateTable(table: Table, allTables: Table[]): ValidationIssue[] {
    const issues: ValidationIssue[] = [];
  ...
  validateCrossTableRelations(tables: Table[]): ValidationIssue[] {
    const issues: ValidationIssue[] = [];
  ...
  validateSingleTable(table: Table): ValidationResult {
    const issues = this.validateTable(table, [table]);
    return this.createValidationResult(issues);
  }

  validateSingleColumn(column: any, tableName: string): ValidationResult {
    const issues: ValidationIssue[] = [];
  ...
  private filterIssues(issues: ValidationIssue[]): ValidationIssue[] {
  ...
  private applySeverityOverride(issue: ValidationIssue): ValidationIssue {
