...
class SecurityValidator {
    constructor() {
        ...
        this.encryptionKeywords = ['encrypted', 'hashed', 'secure', 'protected'];
    }
    validateSensitiveData(table) {
        const issues = [];
        for (const column of table.columns) {
            const columnName = column.name.toLowerCase();
            for (const keyword of this.sensitiveKeywords) {
                if (columnName.includes(keyword)) {
                    const isEncrypted = this.encryptionKeywords.some(enc => { var _a; return columnName.includes(enc) || ((_a = column.comment) === null || _a === void 0 ? void 0 : _a.toLowerCase().includes(enc)); });
                    if (!isEncrypted) {
                        issues.push({
    ...
    validatePasswordStorage(column, tableName) {
        const issues = [];
    ...
    validatePIIData(table) {
        ...
        if (piiColumns.length > 0) {
            const hasAuditColumns = table.columns.some(col => ['created_at', 'updated_at', 'created_by', 'updated_by'].includes(col.name.toLowerCase()));
            if (!hasAuditColumns) {
                issues.push({
    ...
    validateSQLInjectionRisk(table) {
        const issues = [];
        for (const column of table.columns) {
    ...
    validateDataRetention(table) {
        var _a, _b, _c;
        const issues = [];
        const hasTimestamp = table.columns.some(col => ['created_at', 'timestamp', 'date'].some(keyword => col.name.toLowerCase().includes(keyword)));
        if (hasTimestamp) {
            const hasRetentionPolicy = ((_a = table.comment) === null || _a === void 0 ? void 0 : _a.toLowerCase().includes('retention')) ||
                ((_b = table.comment) === null || _b === void 0 ? void 0 : _b.toLowerCase().includes('ttl')) ||
                ((_c = table.comment) === null || _c === void 0 ? void 0 : _c.toLowerCase().includes('expire'));
    ...
    validateAccessControl(table) {
        var _a, _b;
        const issues = [];
        const hasPermissionColumns = table.columns.some(col => ['role', 'permission', 'access', 'owner', 'user_id'].some(keyword => col.name.toLowerCase().includes(keyword)));
        if (hasPermissionColumns) {
            const hasRowLevelSecurity = ((_a = table.comment) === null || _a === void 0 ? void 0 : _a.toLowerCase().includes('rls')) ||
                ((_b = table.comment) === null || _b === void 0 ? void 0 : _b.toLowerCase().includes('row level security'));
    ...
    validateForeignKeyConstraints(table) {
        const issues = [];
        for (const fk of table.foreignKeys) {
            if (fk.onDelete === 'CASCADE' || fk.onDelete === 'SET NULL') {
                const isSensitiveTable = ['user', 'account', 'profile', 'payment'].some(keyword => fk.referenceTable.toLowerCase().includes(keyword));
                if (isSensitiveTable) {
                    issues.push({
