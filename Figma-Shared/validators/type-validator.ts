import { Column, Table } from '../types/database.types';
import { ValidationIssue, ValidationCategory } from '../types/validation.types';

export class TypeValidator {
  private typeRecommendations: Map<string, string[]> = new Map([
  ...
  private incompatibleTypes: Map<string, string[]> = new Map([
  ...
  validateColumnType(column: Column, tableName: string): ValidationIssue[] {
  ...
  validateTypeLength(column: Column, tableName: string): ValidationIssue[] {
    const issues: ValidationIssue[] = [];
  ...
  validateNumericPrecision(column: Column, tableName: string): ValidationIssue[] {
    const issues: ValidationIssue[] = [];
  ...
  validateTypeCompatibility(table: Table): ValidationIssue[] {
    const issues: ValidationIssue[] = [];
      ...
      if (incompatible) {
        for (const otherColumn of table.columns) {
          if (column.name !== otherColumn.name && incompatible.includes(otherColumn.dataType)) {
            const hasRelation = table.foreignKeys.some(
              fk => fk.columns.includes(column.name) && fk.referenceColumns.includes(otherColumn.name)
            );
  ...
  validateTimestampTimezone(column: Column, tableName: string): ValidationIssue[] {
    const issues: ValidationIssue[] = [];
  ...
  validateTextVsVarchar(column: Column, tableName: string): ValidationIssue[] {
    const issues: ValidationIssue[] = [];
  ...
  validateBooleanNaming(column: Column, tableName: string): ValidationIssue[] {
    const issues: ValidationIssue[] = [];
