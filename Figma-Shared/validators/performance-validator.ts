import { Table, Index, ForeignKey } from '../types/database.types';
import { ValidationIssue, ValidationCategory } from '../types/validation.types';

export class PerformanceValidator {
  private maxColumnsPerTable = 100;
  private maxForeignKeysPerTable = 50;
  private maxIndexColumns = 5;
  private recommendedIndexColumns = 3;

  validateTableStructure(table: Table): ValidationIssue[] {
  ...
  validateIndexUsage(table: Table): ValidationIssue[] {
    const issues: ValidationIssue[] = [];
  ...
  validateIndexDesign(index: Index, table: Table): ValidationIssue[] {
    const issues: ValidationIssue[] = [];
    ...
    if (index.columns.length === 1 && index.unique) {
      const column = table.columns.find((c: any) => c.name === index.columns[0]);
      if (column && column.constraints.some((c: any) => c.type === 'PRIMARY_KEY')) {
        issues.push({
  ...
  validateRedundantIndexes(table: Table): ValidationIssue[] {
    const issues: ValidationIssue[] = [];
  ...
  validateNPlusOnePotential(table: Table, allTables: Table[]): ValidationIssue[] {
    const issues: ValidationIssue[] = [];

    for (const fk of table.foreignKeys) {
      const referencedTable = allTables.find(t => t.name === fk.referenceTable);
      if (referencedTable) {
        const reverseFks = referencedTable.foreignKeys.filter(
          r => r.referenceTable === table.name
        );
  ...
  validateLargeTextColumns(table: Table): ValidationIssue[] {
    const issues: ValidationIssue[] = [];

    const largeTextColumns = table.columns.filter(
      col => ['text', 'character varying', 'varchar'].includes(col.dataType) &&
  ...
  validateMissingIndexes(table: Table): ValidationIssue[] {
    const issues: ValidationIssue[] = [];
  ...
  validateCascadeDeletes(table: Table): ValidationIssue[] {
    const issues: ValidationIssue[] = [];
  ...
  private isIndexPrefix(index1: Index, index2: Index): boolean {
