import { DesignRelation, DesignTable } from '../types/design.types';
import { ForeignKey, Table } from '../types/database.types';

export class RelationParser {
  private relationKeywords = [
    'relation', 'foreign', 'reference', 'ref', 'link', 'connect',
    '关联', '外键', '引用', '连接'
  ];

  parseRelationsFromDesign(tables: DesignTable[]): DesignRelation[] {
  ...
  parseRelationsFromSchema(tables: Table[]): ForeignKey[] {
  ...
  inferRelations(tables: DesignTable[]): DesignRelation[] {
    const relations: DesignRelation[] = [];
  ...
  private extractRelationsFromFields(table: DesignTable): DesignRelation[] {
    const relations: DesignRelation[] = [];
  ...
  private isRelationField(field: any): boolean {
  ...
  private createRelationFromField(table: DesignTable, field: any): DesignRelation | null {
  ...
  private extractTargetTableName(fieldName: string): string | null {
  ...
  private inferRelationType(field: any): 'one-to-one' | 'one-to-many' | 'many-to-many' {
    const name = field.name.toLowerCase();
    const type = field.type?.toLowerCase() || '';
  ...
  private inferRelationsBetweenTables(sourceTable: DesignTable, targetTable: DesignTable): DesignRelation[] {
    const relations: DesignRelation[] = [];
  ...
  private deduplicateRelations(relations: DesignRelation[]): DesignRelation[] {
    const seen = new Set<string>();
    const unique: DesignRelation[] = [];
  ...
  validateRelations(relations: DesignRelation[], tables: DesignTable[]): string[] {
    const errors: string[] = [];
  ...
  detectCircularDependencies(relations: DesignRelation[]): string[][] {
  ...
  private buildRelationGraph(relations: DesignRelation[]): Record<string, string[]> {
  ...
  suggestRelationOptimizations(relations: DesignRelation[]): string[] {
