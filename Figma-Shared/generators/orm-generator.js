...
exports.ORMGenerator = void 0;
class ORMGenerator {
    constructor(config) {
        this.config = Object.assign({ indent: '  ', generateRelations: true, generateValidation: true }, config);
    }
    generate(schema) {
    ...
    generatePrismaField(column, table) {
    ...
    generatePrismaRelation(foreignKey, table) {
    ...
    generateTypeORMField(column, table) {
        const decorators = [];
        const isPrimaryKey = table.primaryKeys.includes(column.name);
        const isUnique = column.constraints.some(c => c.type === 'UNIQUE');
        if (isPrimaryKey) {
    ...
    generateTypeORMRelation(foreignKey, table) {
        const relationName = this.toCamelCase(foreignKey.referenceTable);
    ...
    generateSequelizeField(column, table) {
        const props = [];
        const isPrimaryKey = table.primaryKeys.includes(column.name);
        const isUnique = column.constraints.some(c => c.type === 'UNIQUE');
    ...
    mapToSequelizeType(postgresType) {
        const typeMapping = {
            'smallint': 'SMALLINT',
            'integer': 'INTEGER',
            'bigint': 'BIGINT',
    ...
    generateDrizzleField(column, table) {
        const isPrimaryKey = table.primaryKeys.includes(column.name);
        const isUnique = column.constraints.some(c => c.type === 'UNIQUE');
...
exports.ORMGenerator = ORMGenerator;
