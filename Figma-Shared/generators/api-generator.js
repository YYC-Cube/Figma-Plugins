...
class APIGenerator {
    constructor(config) {
        this.config = Object.assign({ generateCRUD: true, generateValidation: true, generateTypes: true, basePath: '/api', indent: '  ' }, config);
    }
    generate(schema) {
        const files = new Map();
    ...
    generateExpressController(table) {
        const tableName = table.name;
        const resourceName = this.toPascalCase(tableName);
        const lines = [];
        const indent = this.config.indent;
    ...
    generateFastifyController(table) {
        const tableName = table.name;
        const resourceName = this.toPascalCase(tableName);
        const lines = [];
        const indent = this.config.indent;
    ...
    generateNestJSController(table) {
        const tableName = table.name;
        const resourceName = this.toPascalCase(tableName);
        const lines = [];
        const indent = this.config.indent;
