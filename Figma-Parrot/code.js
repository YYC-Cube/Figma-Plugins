...
// 线性复制
function duplicateLinear(count, direction, spacing) {
...
// 网格复制
function duplicateGrid(rows, columns, horizontalSpacing, verticalSpacing) {
  const selection = figma.currentPage.selection;
  if (selection.length === 0) {
    figma.ui.postMessage({ type: 'error', message: '请先选择要复制的元素' });
    return;
...
// 圆形复制
function duplicateCircular(count, radius, startAngle, endAngle) {
  const selection = figma.currentPage.selection;
  if (selection.length === 0) {
    figma.ui.postMessage({ type: 'error', message: '请先选择要复制的元素' });
    return;
...
// 随机分布复制
function duplicateRandom(count, areaWidth, areaHeight) {
  const selection = figma.currentPage.selection;
  if (selection.length === 0) {
    figma.ui.postMessage({ type: 'error', message: '请先选择要复制的元素' });
    return;
...
// 应用变换
function applyTransform(transformType, startValue, endValue) {
  const selection = figma.currentPage.selection;
  if (selection.length === 0) {
