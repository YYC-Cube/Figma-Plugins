...
// 生成代码
function generateCode(exportType: string, options: string[]) {
...
// 预览代码
function previewCode(exportType: string) {
  const exportOption = exportOptions.find(e => e.type === exportType);
  if (!exportOption) return;
...
// 复制代码
function copyCode(exportType: string) {
  const exportOption = exportOptions.find(e => e.type === exportType);
  if (!exportOption) return;
...
// 下载代码
function downloadCode(exportType: string, format: string) {
  const exportOption = exportOptions.find(e => e.type === exportType);
  if (!exportOption) return;
...
// 生成示例代码
function generateSampleCode(exportType: string, options: string[]): string {
  switch (exportType) {
    case 'HTML':
      return `<!DOCTYPE html>
<html lang="zh-CN">
