/**
 * @file Figma Figmotion 插件
 * @description 为Figma添加动画效果的插件
 * @module figma-figmotion
 * @author YYC³
 * @version 1.0.0
 * @created 2025-01-30
 */
...
// 动画类型
type AnimationType = 'fade' | 'move' | 'scale' | 'rotate' | 'custom';

// 缓动函数类型
type EasingFunction = 'linear' | 'ease-in' | 'ease-out' | 'ease-in-out' | 'bounce' | 'elastic';
...
// 创建动画
function createAnimation(animationData: any) {
...
// 更新动画
function updateAnimation(animationId: string, animationData: any) {
...
// 批量应用动画
function batchApply(animationTemplate: any, nodeIds: string[]) {
...
// 导出动画
function exportAnimation(animationId: string, format: string) {
  const animation = state.animations.find(anim => anim.id === animationId);
  if (!animation) return;
...
// 应用关键帧
function applyKeyframes(node: SceneNode, keyframes: Keyframe[], progress: number) {
  // 找到当前进度对应的关键帧范围
  ...
  Object.keys(endKeyframe.properties).forEach(prop => {
    const startValue = startKeyframe!.properties[prop as keyof typeof startKeyframe.properties] || 0;
    const endValue = endKeyframe.properties[prop as keyof typeof endKeyframe.properties] || 0;
...
// 应用属性
function applyProperties(node: SceneNode, properties: any) {
...
// 恢复状态
function restoreState(node: SceneNode, originalState: any) {
...
// 生成CSS动画
function generateCSSAnimation(animation: Animation): string {
  // 生成关键帧
...
// 生成SVG动画
function generateSVGAnimation(animation: Animation): string {
  // 简化的SVG动画生成
