...
// 均匀分布
function distributeEvenly(axis: 'horizontal' | 'vertical') {
  const nodes = figma.currentPage.selection;
  if (nodes.length < 3) {
...
function resizeToFit() {
  ...
  nodes.forEach(node => {
    if (node.type === 'TEXT') {
      ...
      // 计算内容边界
      const contentBounds = getBoundingBox(containerNode.children);
      
      // 调整容器大小
      containerNode.resize(contentBounds.width, contentBounds.height);
...
// 批量重命名
function batchRename(pattern: string, startIndex: number) {
  const nodes = figma.currentPage.selection;
  if (nodes.length === 0) {
    figma.notify('请先选择元素');
    return;
...
// 网格复制
function duplicateGrid(rows: number, columns: number, xSpacing: number, ySpacing: number) {
  const nodes = figma.currentPage.selection;
  if (nodes.length === 0) {
    figma.notify('请先选择元素');
    return;
...
// 生成占位符
function generatePlaceholders(count: number, type: 'text' | 'image' | 'rectangle') {
  const placeholders: SceneNode[] = [];
  
  for (let i = 0; i < count; i++) {
    let placeholder: SceneNode;
    
    switch (type) {
      case 'text':
        placeholder = figma.createText();
        (placeholder as TextNode).characters = `Placeholder ${i + 1}`;
...
function extractStyles() {
  ...
  nodes.forEach(node => {
    ...
    if (node.type === 'TEXT') {
      ...
      style.fontName = textNode.fontName;
      style.fontSize = textNode.fontSize;
      style.lineHeight = textNode.lineHeight;
      style.letterSpacing = textNode.letterSpacing;
      style.textCase = textNode.textCase;
      style.textDecoration = textNode.textDecoration;
      stylesCreated++;
    }
  });
...
// 辅助函数：获取边界框
function getBoundingBox(nodes: SceneNode[]): { x: number; y: number; width: number; height: number } {
  if (nodes.length === 0) {
