...
// 创建流程图
function createFlow(flowTypeId, nodes, directionId) {
...
// 连接选中的节点
function connectSelectedNodes(connectorStyleId, directionId) {
...
// 自动排列流程图
function autoArrangeFlow(directionId) {
  const direction = layoutDirections.find(d => d.id === directionId);
  if (!direction) return;
...
// 添加决策点
function addDecisionPoint(position) {
  // 模拟添加决策点
  setTimeout(() => {
    figma.ui.postMessage({
      type: 'decision-added',
