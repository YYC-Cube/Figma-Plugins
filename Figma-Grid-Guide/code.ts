/**
 * @file Figma Grid Guide 插件
 * @description 为Figma添加网格和参考线的插件
 * @module figma-grid-guide
 * @author YYC³
 * @version 1.0.0
 * @created 2025-01-30
 */
...
type GridType = 'column' | 'row' | 'baseline' | 'custom';
...
function showUI() {
  state.selectedFrame = figma.currentPage.selection.find(node => node.type === 'FRAME') as FrameNode || null;
...
function createGrid(gridConfig: GridConfig) {
...
function updateGrid(gridId: string, gridConfig: GridConfig) {
...
function applyGridToFrame(grid: GridConfig) {
...
function createColumnGridStyle(grid: GridConfig): string {
...
function createRowGridStyle(grid: GridConfig): string {
  const gridStyle = figma.createGridStyle();
  gridStyle.name = grid.name;
  gridStyle.gridColumns = [];
  gridStyle.gridRows = [{
...
function createBaselineGridStyle(grid: GridConfig): string {
  const gridStyle = figma.createGridStyle();
  gridStyle.name = grid.name;
  gridStyle.gridColumns = [];
  gridStyle.gridRows = [{
...
function clearFrameGrids(frame: FrameNode) {
  frame.gridStyleId = '';
}

function createGuides(guides: Guide[]) {
...
function applyGuidesToPage(guides: Guide[]) {
...
function deleteGuides(guideIds: string[]) {
...
function alignToGrid(gridId: string) {
  const grid = state.grids.find(g => g.id === gridId);
  if (!grid) return;
...
function snapToGrid(enabled: boolean) {
